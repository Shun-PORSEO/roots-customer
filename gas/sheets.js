...
function addTaskItem(taskId, lineId, itemName, quantity, memo) {
    const sheet = ensureTaskItemsSheet();
    if (!sheet)
        throw new Error("task_items シートを作成できません");
...
function updateVenue(venueId, patch) {
    const sheet = ensureVenuesSheet();
    if (!sheet)
        return false;
    const data = sheet.getDataRange().getValues();
