 ...
 */
function setupDefaultTaskMaster(venueId: string): { added: number; skipped: number } {
  ...
  for (let i = 1; i < data.length; i++) {
    ...
    if (!baseTaskId) continue;
    if (baseVenueId !== "") continue;   // 基本タスク以外(他式場の固有タスク等)は除外
 ...
 * GASエディタから手動実行する想定。
 */
function syncAllVenuesWithBaseTasks(): { venues: number; added: number; skipped: number } {
