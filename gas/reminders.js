...
function alreadySentToday(sentDrafts, coupleId, taskId, today) {
    for (const d of sentDrafts) {
        if (d.couple_id !== coupleId)
            continue;
        if (d.task_id !== taskId)
            continue;
...
function sendReminders() {
    ...
    for (const venue of venues) {
        ...
        for (const customer of customers) {
            ...
            for (const { task, days } of dueTasks) {
                if (alreadySentToday(sentDrafts, customer.line_id, task.task_id, today))
                    continue;
            ...
            if (overdueTasks.length > 0 && !alreadySentToday(sentDrafts, customer.line_id, OVERDUE_DIGEST_TASK_ID, today)) {
                const lines = overdueTasks.map(({ task, days }) => `・${task.task_content}（${Math.abs(days)}日経過）`);
                const message = `【期限が過ぎているタスクがあります】${coupleName}様\n\n` +
                    lines.join("\n") +
                    `\n\nお早めにご対応をお願いいたします🙇` +
                    REMINDER_CTA;
                const ok = pushLineMessage(customer.line_id, message, venue.line_channel_access_token);
            ...
            if (perCoupleIndividual > 0 || perCoupleOverdueCount > 0) {
                const parts = [];
                if (perCoupleIndividual > 0)
                    parts.push(`個別${perCoupleIndividual}件`);
        ...
        if (venue.planner_line_user_id && reportLines.length > 0) {
            const summary = `【本日のリマインド送信完了】${venue.venue_name}\n\n` +
                reportLines.join("\n") +
    ...
    console.log(`[sendReminders] 完了: 個別 ${totalIndividualSent} 件 / 期限切れまとめ ${totalOverdueDigestSent} 件`);
}
// ─── トリガー管理 ────────────────────────────────────────────────
